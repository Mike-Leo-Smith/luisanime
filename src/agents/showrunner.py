...
class ShowrunnerAgent(BaseOrchestrator):
    def route_macro_flow(self, state: AFCState) -> str:
        ...
        if state.get("escalation_required"):
            print(
                "🎬 [Showrunner] Escalation required. Routing to Director for complexity reduction."
            )
            return "director"
        print(
            f"🎬 [Showrunner] Processing next scene. Remaining: {len(state.get('unprocessed_scenes'))}"
        )
        return "director"
